...
void OverlayLayer::OnEvent(NGN::Event& event)
{
	NGN::EventDispatcher dispatcher(event);
	// Dispatch to another function based on the event type - "OnMouseButtonPressed" func*
...
{
	glm::vec2 framebufferSize = NGN::Application::Get().GetFramebufferSize();
	glm::vec2 mousePos = NGN::Application::Get().GetWindow().GetMousePos();
...
bool OverlayLayer::OnMouseButtonPressed(NGN::MouseButtonPressedEvent& event)
{
	if (!IsButtonHovered())
		return false;
	
	auto voidLayer = NGN::Application::Get().GetLayer<VoidLayer>();
	if (voidLayer)
	{
	...
	{
		auto appLayer = NGN::Application::Get().GetLayer<AppLayer>();
